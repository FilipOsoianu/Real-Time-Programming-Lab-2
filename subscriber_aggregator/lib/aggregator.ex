defmodule Aggregator do
  ...
  def aggregate(aggregator, message, socket, address, port) do
    GenServer.cast(aggregator, {:aggregate, message, socket, address, port})
  end
  ...
  @impl true
  def handle_cast({:aggregate, data, socket, address, port}, state) do
    ...
    if Kernel.length(joined_list) > 0 do
      Enum.each(joined_list, fn element ->
        {:ok, json} = Jason.encode(element)
        :gen_udp.send(socket, address, 6666, json)
      end)
    end
  ...
  defp join_sensors(list_iot, list_sensors, list_legacy_sensors) do
    joined_list =
      Enum.map(list_iot, fn iot_msg ->
        iot_timestamp = iot_msg["unix_timestamp_100us"]

        sensors_msg =
          Enum.find(list_sensors, fn sensors_msg ->
            sensors_timestamp = sensors_msg["unix_timestamp_100us"]

            iot_timestamp - sensors_timestamp <= 200 &&
              iot_timestamp - sensors_timestamp >= -200
          end)

        legacy_sensors_msg =
          Enum.find(list_legacy_sensors, fn legacy_sensors_msg ->
            legacy_sensors_timestamp = legacy_sensors_msg["unix_timestamp_100us"]

            iot_timestamp - legacy_sensors_timestamp <= 200 &&
              iot_timestamp - legacy_sensors_timestamp >= -200
          end)
